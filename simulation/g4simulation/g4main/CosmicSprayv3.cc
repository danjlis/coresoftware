  ...
  : PHG4ParticleGeneratorBase(name)
{
  ...
  _o_x_max = x_max;
  _o_x_min = x_min;
  _mean_x = (_o_x_max + _o_x_min)/2;
  _x_rad = (_o_x_max - _o_x_min)/2;
  _z_rad = 0;
  _debug = debug;
...
void CosmicSpray::set_dimension_offset(const double &o){
  _offset = o;
  _o_x_min = _x_min - _offset;
  _o_x_max = _x_max + _offset;
  _o_z_min = _z_min - _offset;
  _o_z_max = _z_max + _offset;
  _mean_x = (_o_x_max + _o_x_min)/2;
  _x_rad = (_o_x_max - _o_x_min)/2;
  _z_rad = o;
}
...
int CosmicSpray::process_event(PHCompositeNode *topNode)
{
  ...
  while (loop){
    randomGen->SetSeed();
    gun_x = randomGen->Uniform(_o_x_min, _o_x_max);
    gun_z = randomGen->Uniform(_o_z_min, _o_z_max);

    if (fabs(gun_z) < _z_max) gun_y = _y_top_fix * sqrt(1 - TMath::Power((gun_x - _mean_x)/_x_rad,2));
    else {
      if (fabs(gun_z) - _z_max > _z_rad*sqrt(1 - TMath::Power((gun_x - _mean_x)/_x_rad,2))){
        continue;
      }
      else {
        gun_y = _y_top_fix * sqrt(1 - TMath::Power((gun_x - _mean_x)/_x_rad,2)- TMath::Power((fabs(gun_z) - _z_max)/_z_rad,2));
      }
    }
